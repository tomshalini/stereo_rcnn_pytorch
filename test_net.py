...
if __name__ == '__main__':
  ...
  # Checking mmdnn code
  '''load_name = os.path.join(input_dir,
                           'stereo_rcnn_testing_1_6477.pth')
  ...
  #print(model)
  for param_tensor in stereoRCNN.state_dict():
    print(param_tensor, "\t", stereoRCNN.state_dict()[param_tensor].size())
  exit(0)
  ...
  with torch.no_grad():
    ...
    for i in range(num_images):
      ...
      #print("dbdbdbdbdbdbdbdbdbdbbdbdbdbbbbbbbbbbbbbbbbbbbbbbb",roidb[i])
      data = next(data_iter)
      ...
      for keep_inx in range(box_delta_left.size()[0]):                                      # loop for 300 proposals (300 x 8) 4 for left and 4 for right box
      ...
      #################### Code to draw ground truth on image ######################
      #for i in range(roidb[i]['position'].shape[0])
      #for gtbox in range(len(roidb[i]['position']))
      '''gt_xyz=roidb[i]['position'][0]

      gt_dim=roidb[i]['dim_orien'][0][0:3]
      gt_alpha=roidb[i]['dim_orien'][0][-1]
      x = gt_xyz[0]
      z = gt_xyz[2]
      #gt_theta = gt_alpha - np.arctan(-x, z)
      gt_theta = gt_alpha + m.pi / 2 - m.atan2(-x, z)

      #print("ground truth xyz",gt_xyz)
      print("ground truth w h l theta", gt_dim,gt_alpha)
      #print("ground truth theta",gt_alpha)

      im2show_left = vis_utils.vis_single_gt_box_in_img(im2show_left, calib, gt_xyz, gt_dim, gt_theta)'''
            ...
            for solved_idx in range(succ.size(0)):
              if succ[solved_idx] > 0: # succ
                ...
                state_rect, z = box_estimator.solve_x_y_theta_from_kpt(im2show_left.shape, calib, \
                                                  ...
                                                  dis_final[solved_idx].cpu().numpy(), kpts_all[solved_idx].cpu().numpy())######Input is and output x,y ,theta and depth
      ...
      cv2.imwrite('result_smallDataset/'+str(i)+'_.png', im2show)
