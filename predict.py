...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for i in range(num_images):
            ...
            img_left = imdb.img_left_path_at(i)
            img_right= imdb.img_right_path_at(i)
            ...
            for keep_inx in range(
                    box_delta_left.size()[0]):  # loop for 300 proposals (300 x 8) 4 for left and 4 for right box
                        ...
                        for solved_idx in range(succ.size(0)):
                            if succ[solved_idx] > 0:  # succ
                                ...
                                state_rect, z = box_estimator.solve_x_y_theta_from_kpt(im2show_left.shape, calib, \
                                                                                       poses_all[
                                                                                           solved_idx, 7].cpu().numpy(),
                                                                                       dim, box_left, \
                                                                                       dis_final[
                                                                                           solved_idx].cpu().numpy(),
                                                                                       kpts_all[
                                                                                           solved_idx].cpu().numpy())  ######Input is and output x,y ,theta and depth
                                xyz = np.array([state_rect[0], state_rect[1], z])
                                theta = state_rect[2]
                                if score > vis_thresh:
                                    im_box = vis_utils.vis_box_in_bev(im_box, xyz, dim, theta,
