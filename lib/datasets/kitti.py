...
class kitti(imdb):
    ...
    def _load_image_set_index_new(self):
        ...
        if self.name == 'kitti_train':
            ...
            test_set_file = open('data/kitti/splits/test.txt', 'r')
            image_index = test_set_file.read().split('\n')
        image_index=list(filter(None,image_index))
        #print(image_index)
        return image_index
    ...
    def _load_kitti_annotation(self,index):
        if self._image_set == 'test':
            ...
            position= np.zeros((num_objs,3), dtype= np.float32)                      #added by shalini to get position and depth of groundf truth
            ...
            for i in range(len(objects)):
                ...
                gt_classes[i] = cls
                overlaps[i, cls] = 1.0
                position[i,:]=objects[i].pos                                              #added by shalini to get position and depth of groundf truth
