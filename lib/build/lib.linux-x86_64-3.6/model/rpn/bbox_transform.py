...
def kpts_transform_inv(boxes, kpts_deltas, grid):
    # kpts_deltas: 1 x num x 1
    widths = boxes[:, :, 2] - boxes[:, :, 0] + 1.0
    ctr_x = boxes[:, :, 0]
    ...
    pred_kpts = kpts_delta * widths/grid + ctr_x

    return pred_kpts.unsqueeze(2), kpts_type.unsqueeze(2)

def border_transform_inv(boxes, border_deltas, grid):
