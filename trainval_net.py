...
if __name__ == '__main__':
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    save_checkpoint({
      'epoch': epoch + 1,
      'model':stereoRCNN,
      'model_state': stereoRCNN.state_dict(),
      'optimizer': optimizer.state_dict(),
      'uncert':uncert.data,
    }, save_name)
    save_name_testing=os.path.join(output_dir, 'stereo_rcnn_save_18.04_{}_{}.pth'.format(epoch, step))
    torch.save(stereoRCNN ,save_name_testing)
    
    save_stereo_tar=os.path.join(output_dir, 'stereoRCNN_18.04_{}_{}.tar'.format(epoch,step))
    torch.save(dict(model=stereoRCNN,
                   model_state=stereoRCNN.state_dict()),
                   save_stereo_tar)
    log_string('save model: {}'.format(save_tar))
    end = time.time()
